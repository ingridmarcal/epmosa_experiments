...
output_file="/home/ingridmarcal/Documents/statistical_analisys_exponential/test_budgets_execution_times_lang_dynamosa.txt"
...
for ((i=0; i<${#identifiers[@]}; i++)); do
    ...
    for ((nb=1; nb<=${n_bugs[$i]}; nb++)); do
        ...
        echo "##### GEN TESTS EXECUTION" | tee -a "$output_file"Cloud08j
		echo "" | tee -a "$output_file"
