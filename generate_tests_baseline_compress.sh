...
output_file="/home/ingridmarcal/Documents/gen_times_dynamosa_compress.txt"
...
for ((i=0; i<${#identifiers[@]}; i++)); do
    ...
    for ((nb=1; nb<=${n_bugs[$i]}; nb++)); do
        ...
        for nv in "${n_versions[@]}"; do
            ...
            perl "/home/ingridmarcal/Documents/defects4j/framework/bin/gen_tests.pl" -g evosuite -p "${identifiers[$i]}" -v "${nb}b" -n "${nv}" -o ./evosuite_tests_dynamosa -b 93
			end_time_gen=$(date +%s)
