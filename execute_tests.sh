...
output_file="/home/ingridmarcal/Documents/test_budgets_execution_times_codec_nb.txt"
...
for ((i=0; i<${#identifiers[@]}; i++)); do
    ...
    for ((nb=1; nb<=${n_bugs[$i]}; nb++)); do
        ...
        for ((j=0; j<${#n_versions[@]}; j++)); do
            ...
            command2_output=$(perl "/home/ingridmarcal/Documents/defects4j/framework/bin/defects4j" test -s "./evosuite_tests_nb/${identifiers[$i]}/evosuite/$nv/${identifiers[$i]}-${nb}b-evosuite.$nv.tar.bz2" 2>&1) # Adjusted for clarity and correctness
            end_time_gen=$(date +%s)
