...
identifiers=("Mockito")
project_names=("mockito")
n_bugs=(38)
n_versions=(1 2 3 4 5)

# File to store the results
output_file="/home/ingridmarcal/Documents/gen_times_mockito_bme.txt"
...
for ((i=0; i<${#identifiers[@]}; i++)); do
    ...
    for ((nb=1; nb<=${n_bugs[$i]}; nb++)); do
        cd "/home/ingridmarcal/Documents/PrimeiraGeracao/${project_names[$i]}/"
        ...
        for nv in "${n_versions[@]}"; do
            ...
            perl "/home/ingridmarcal/Documents/defects4j/framework/bin/gen_tests.pl" -g evosuite -p "${identifiers[$i]}" -v "${nb}b" -n "${nv}" -o ./evosuite_tests_bme -b 10
			end_time_gen=$(date +%s)
